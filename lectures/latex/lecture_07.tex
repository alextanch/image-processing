...
\begin{itemize}
    \item Координаты точки $\mathbf{w}_2$ во второй системе координат связаны с координатами
    точки $\mathbf{w}_1$ в первой системе координат следующим образом
    $$
...
$$
    \mathbf{w}_2 = \mathbf{R}\cdot\mathbf{w}_1 + \mathbf{t}
$$
Вектор сдвига $\mathbf{t}$ равен вектору, соединяющему начала отсчетов двух систем координат:
$$
    \mathbf{t} = \mathbf{o}_1 - \mathbf{o}_2
$$
...
\end{tikzpicture}
\end{center}
Столбцы матрицы вращения $\mathbf{R}$ --- это координаты осей первой системы координат во второй системе координат:
$$
    \mathbf{R} =
    \begin{bmatrix}
            r_{11} & r_{12} & r_{13} \\
...
\frametitle{Пример преобразования однородных координат}
    ...
    $$
        ...
        \widetilde{\mathbf{w}}_1    
    $$
    Найти, как преобразуются декартовы координаты точек и
...
\end{figure}
\begin{itemize}
    \item Из правдоподобия треугольников:
    \begin{align*}
